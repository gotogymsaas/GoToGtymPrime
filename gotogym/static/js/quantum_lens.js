...
(function () {
  ...
  var campos = document.querySelectorAll('[data-quantum-lens]');
  ...
  Array.prototype.forEach.call(campos, function (campo) {
    ...
    contenedor.addEventListener('pointermove', function (evento) {
      ...
      campo.style.setProperty('--ql-x', x.toFixed(2) + 'px');
      campo.style.setProperty('--ql-y', y.toFixed(2) + 'px');
    });

    contenedor.addEventListener('pointerleave', function () {
      campo.style.setProperty('--ql-x', '0px');
      campo.style.setProperty('--ql-y', '0px');
    });
  });
})();
